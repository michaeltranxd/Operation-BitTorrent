...
ssize_t send_file(char* filename, int sockfd, int index, int filesize){
	...
	if (ret[0] != 0) { // at least one segment is missing. Return ret (and request the missing segments)
		...
		while (index < (segment_count + 1)) {
			...
			if (fwrite(addr[index], sizeof(char), segment[index], fp_combined) < segment[index]) {
				printf("fwrite() ends before sending %zu bytes to file %s from segment indexed %d\n", segment[index], base_filename, index);
				printf("%s\n", strerror(errno));
				free(ret);
				fclose(fp_combined);
				exit(1);
...
size_t* schedule_segment_size (size_t filesize, int segment_count) {
	size_t *segment = (size_t *) malloc(segment_count * sizeof(size_t));
